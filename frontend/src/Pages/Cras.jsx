...
const Cras = () => {
  ...
  const searchCarAvailability = async () => {
    try {
      const { data } = await axios.post('/api/bookings/check-availability', {   // ✅ spelling
  ...
  }, [pickupDate, pickupLocation, returnDate]);   // ✅ correct deps

  const displayedCars = filteredCars.filter((car) =>
  (car?.name?.toLowerCase() || "").includes(input.toLowerCase()) ||
  (car?.model?.toLowerCase() || "").includes(input.toLowerCase()) ||
  (car?.features &&
    car.features.some((f) => (f?.toLowerCase() || "").includes(input.toLowerCase())))
);
    ...
    <div className="bg-gray-100 min-h-screen">
      ...
      <div className="px-6 md:px-16 lg:px-24 xl:px-32 pb-20">
        <p className="text-gray-600 text-sm mb-4">
          Showing {displayedCars.length} car{displayedCars.length !== 1 ? 's' : ''}
        </p>
